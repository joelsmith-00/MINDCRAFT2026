...
class SkillRegistry:
    ...
    def load_skills(self, skills_dir: str, context: Dict[str, Any] = None, skip_skills: List[str] = None):
        """Dynamically load skills from the specified directory.
        
        Args:
            skills_dir: Path to skills directory
            context: Optional context dict for skills
            skip_skills: List of skill filenames to skip (e.g., ['detection_skill.py'])
        """
        ...
        for filename in os.listdir(skills_dir):
            if filename.endswith(".py") and filename != "__init__.py":
                # Skip specified skills
                if filename in skip_skills:
                    print(f"Skipped skill: {filename[:-3]}")
                    continue
